...
public class ProcessNotStartWithType {
    public static void processNotStartWithType(StringProcessEntity processEntity, StringProcessChildEntity childEntity, List<String> listWord, AccountObject customer, StringProcessChildEntity previousEntity,int position) {
        ...
        if (listWord.contains("x")) { //Nếu chứa x
            if (listWord.lastIndexOf("x") == listWord.size() - 1 //Nếu x ở cuối cùng
                    || listWord.lastIndexOf("x") < listWord.size() - 2) { //Nếu sau x có nhiều từ
                setError(processEntity, childEntity, "Không đúng định dạng. Cần ghi rõ là Đề(de), Lô(lo), Xiên(xien) hay là Ba càng(ba cang, bacang,3c)");
                ...
                if (patternPriceFull.matcher(price).matches()) { //Nếu đúng định dạng giá trị tiền
                    ...
                    if (priceValue < 0) { //Nếu giá trị tiền = 0
                        ...
                        if (listWord.size() < 3) { //Nếu số từ trong câu nhỏ hơn 2
                            setError(processEntity, childEntity, "Không đúng định dạng. Cần ghi rõ các cặp số");
                        } else {
                            switch (priceUnit) {
                                case 3:
                                    type = TypeEnum.TYPE_LO;
                                    childEntity.setType(type);
                                    break;
                                default:
                                    type = TypeEnum.TYPE_DE;
                                    childEntity.setType(type);
                            }

                            if (type == TypeEnum.TYPE_DE) {
                                for (int i = 0; i < listWord.size() - 2; i++) { //Lấy các cặp số
...
//                                                processAddNumberObject(childEntity, type, priceValue, word);
                                ...
                                if (isError && previousEntity != null) {
                                    childEntity.setError(false);
                                    boolean hasError = false;
                                    for (int i = 0; i < position; i++) {
                                        if (processEntity.getListChild().get(i).isError()){
                                            hasError = true;
                                            break;
                                        }
                                    }
                                    processEntity.setHasError(hasError);
                                    childEntity.setValue(Html.fromHtml(TypeEnum.getStringByType(previousEntity.getType()) + " " + childEntity.getValue()).toString());
                                    StringCalculate.processChildEntity(processEntity, childEntity, customer, previousEntity,position);
                                }
                            } else if (type == TypeEnum.TYPE_LO) {
                                for (int i = 0; i < listWord.size() - 2; i++) { //Lấy các cặp số
                                    String word = listWord.get(i);
                                    if (patternNumber.matcher(word).matches()) { //Nếu từ là số
                                        if (word.length() == 1 || word.length() > 4) {
                                            setError(processEntity, childEntity, "Lô phải là các cặp số:01 02 03...232 sẽ hiểu là 23 và 32. 2345 sẽ hiểu là 23 và 45");
                                            isError = true;
                                            break;
                                        } else if (word.length() == 3) {
                                            if (Pattern.compile("(\\d)\\d*\\1").matcher(word).matches()) { //Nếu là số đối xứng
...
//                                                processAddNumberObject(childEntity, type, priceValue, word);
                                        ...
                                        switch (word) {
                                                    ...
                                                    if (i + 2 >= listWord.size() - 2) {
                                                        ...
                                                        for (int j = i + 2; j < listWord.size() - 2; j++) { //duyệt các từ sau
                                                            ...
                                                            if (patternNumber.matcher(wordAfter2).matches()) { //Nếu là số
                                                                if (wordAfter2.length() > 1) { //Nếu là số có từ 2 chữ số trở lên
                                                                    isError = true;
                                                                    setError(processEntity, childEntity, "Sau đít không thể là Lô nữa\n-đít1 2 3 hoặc đít1.2.3 hoặc đít123 được hiểu là đít1 đít2 đít3");
                                                                    break;
                                                                } else { //Nếu là số có 1 chữ số
                                                                    wordAfter1 = wordAfter1 + wordAfter2;
                                                                    if (!checkValidateRepeatChar(wordAfter1)) {
                                                                        isError = true;
                                                                        setError(processEntity, childEntity, "Các số trong Đầu, Đuôi, Tổng không được trùng nhau. Ví dụ: Lô đầu 2345 x 10n");
                                                                        break;
                                                                    }
                                                                }
                                                            } else { // nếu là chữ
                                                    ...
                                                    if (i + 2 >= listWord.size() - 2) {
                                                        ...
                                                        for (int j = i + 2; j < listWord.size() - 2; j++) { //duyệt các từ sau
                                                            ...
                                                            if (patternNumber.matcher(wordAfter2).matches()) { //Nếu là số
                                                                if (wordAfter2.length() > 1) { //Nếu là số có từ 2 chữ số trở lên
                                                                    isError = true;
                                                                    setError(processEntity, childEntity, "Sau đầu không thể là Lô nữa\n-đầu1 2 3 hoặc đầu1.2.3 hoặc đầu123 được hiểu là đầu1 đầu2 đầu3");
                                                                    break;
                                                                } else { //Nếu là số có 1 chữ số
                                                                    wordAfterDau = wordAfterDau + wordAfter2;
                                                                    if (!checkValidateRepeatChar(wordAfterDau)) {
                                                    ...
                                                    if (i + 2 >= listWord.size() - 2) {
                                                        ...
                                                        for (int j = i + 2; j < listWord.size() - 2; j++) { //duyệt các từ sau
                                                            ...
                                                            if (patternNumber.matcher(wordAfter2).matches()) { //Nếu là số
                                                                if (wordAfter2.length() > 1) { //Nếu là số có từ 2 chữ số trở lên
                                                                    isError = true;
                                                                    setError(processEntity, childEntity, "Sau tổng không thể là Lô nữa\n-tổng1 2 3 hoặc tổng1.2.3 hoặc tổng123 được hiểu là tổng1 tổng2 tổng3");
                                                                    break;
                                                                } else { //Nếu là số có 1 chữ số
                                                                    wordAfterTong = wordAfterTong + wordAfter2;
                                                                    if (!checkValidateRepeatChar(wordAfterTong)) {
                                            ...
                                            case "chamlt":
                                                ...
                                                if (patternNumber.matcher(wordAfterchamlt).matches()) { //Nếu từ là số
                                                    if (!checkValidateRepeatChar(wordAfterchamlt) || wordAfterchamlt.length() > 5) {
                                                        isError = true;
                                                        setError(processEntity, childEntity, "Không đúng định dạng. Sau chạm chỉ chấp nhận từ 1 đến 5 số liền nhau và khác nhau. Ví dụ: lo cham12345 x10n");
                                                        break;
                                                    }
                                                    if (i + 2 >= listWord.size() - 2) {
                                                        i += 1;
                                                        ...
                                                        for (int j = i + 2; j < listWord.size() - 2; j++) { //duyệt các từ sau
                                                            ...
                                                            if (patternNumber.matcher(wordAfter2).matches()) { //Nếu là số
                                                                if (wordAfter2.length() > 1) { //Nếu là số có từ 2 chữ số trở lên
                                                                    isError = true;
                                                                    setError(processEntity, childEntity, "Sau chạm không thể là Lô nữa\n-chạm1 2 3 hoặc chạm1.2.3 hoặc chạm123 được hiểu là chạm1 chạm2 chạm3");
                                                                    break;
                                                                } else { //Nếu là số có 1 chữ số
                                                                    wordAfterchamlt = wordAfterchamlt + wordAfter2;
                                                                    if (!checkValidateRepeatChar(wordAfterchamlt) || wordAfterchamlt.length() > 5) {
                                            ...
                                            case "gheptrong":
                                                ...
                                                if (patternNumber.matcher(wordAfterGhepTrong).matches()) { //Nếu từ là số
                                                    if (!checkValidateRepeatChar(wordAfterGhepTrong)) {
                                                        isError = true;
                                                        setError(processEntity, childEntity, "Các số không được trùng nhau. Ví dụ: Lô ghép trong 2345 x 10n");
                                                        break;
                                                    } else {
                                                        i += 1;
                                                    }
                                ...
                                if (isError && previousEntity != null) {
                                    ...
                                    processEntity.setHasError(hasError);
                                    childEntity.setValue(TypeEnum.getStringByType(previousEntity.getType()) + " " + previousEntity.getValue());
                                    StringCalculate.processChildEntity(processEntity, childEntity, customer, previousEntity,position);
                                }
                            }
            ...
            if (patternPriceFull.matcher(price).matches()) { //Nếu đúng định dạng giá trị tiền
                ...
                if (priceValue < 0) { //Nếu giá trị tiền = 0
                    ...
                    if (listWord.size() < 2) { //Nếu số từ trong câu nhỏ hơn 2
                        setError(processEntity, childEntity, "Không đúng định dạng. Cần ghi rõ các cặp số");
                    } else {
                        switch (priceUnit) {
                            case 3:
                        ...
                        if (type == TypeEnum.TYPE_DE) {
                            for (int i = 0; i < listWord.size() - 1; i++) { //Lấy các cặp số
                                String word = listWord.get(i);
                                if (patternNumber.matcher(word).matches()) { //Nếu từ là số
                                    if (word.length() == 1 || word.length() > 4) {
                                        setError(processEntity, childEntity, "Đề phải là các cặp số:01 02 03...232 sẽ hiểu là 23 và 32. 2345 sẽ hiểu là 23 và 45");

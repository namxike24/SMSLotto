...
public class ProcessStartWithXienGhep2 {
    public static void processStartWithTypeXienGhep2(StringProcessEntity processEntity, StringProcessChildEntity childEntity, List<String> listWord, AccountObject customer, StringProcessChildEntity previousEntity,int position) {
        try {
            ...
            int type = TypeEnum.TYPE_XIENGHEP2;
                    ...
                    if (patternPrice.matcher(price).matches()) { //Nếu đúng định dạng giá trị tiền
                        ...
                        if (priceValue < 0) { //Nếu giá trị tiền = 0
                            ...
                            if (listWord.size() < 4) { //Nếu số từ trong câu nhỏ hơn 4
                                setError(processEntity, childEntity, "Không đưa ra được số cược xiên. Vì tránh trường hợp tràn bộ nhớ nên Xiên ghép 2 chỉ chấp nhận từ 3 đến 20 cặp số. Các cặp số đó không được giống nhau");
                                isError = true;
                            } else {

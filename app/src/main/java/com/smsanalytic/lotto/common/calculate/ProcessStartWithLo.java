...
public class ProcessStartWithLo {

    public static void processStartWithTypeLo(StringProcessEntity processEntity, StringProcessChildEntity childEntity, List<String> listWord, AccountObject customer, StringProcessChildEntity previousEntity,int position) {
        try {
            ...
            if (listWord.contains("x")) { //Nếu chứa x
                if (listWord.lastIndexOf("x") == listWord.size() - 1) { //Nếu x ở cuối cùng
                    ...
                    if (patternPrice.matcher(price).matches()) { //Nếu đúng định dạng giá trị tiền
                        ...
                        if (priceUnit == -1) {
                            priceUnit = PriceUnitEnum.PRICE_D;
                        }else {
                        ...
                        if (priceUnit != PriceUnitEnum.PRICE_D && (customer == null || accountSetting.getChapnhandonvilolanghin() == 1)) {
...
//                                                processAddNumberObject(childEntity, type, priceValue, word);
                ...
                if (previousEntity!=null && !previousEntity.isError() && childEntity.getType()==type){
                    String moneyAdd = ((int)previousEntity.getListDataLoto().get(0).getMoneyTake())+"d";

...
public class ProcessStartWithXienGhep4 {
    public static void processStartWithTypeXienGhep4(StringProcessEntity processEntity, StringProcessChildEntity childEntity, List<String> listWord, AccountObject customer, StringProcessChildEntity previousEntity,int position) {
        try {
            ...
            int type = TypeEnum.TYPE_XIENGHEP4;
                    ...
                    if (patternPrice.matcher(price).matches()) { //Nếu đúng định dạng giá trị tiền
                        ...
                        if (priceValue < 0) { //Nếu giá trị tiền = 0
                            ...
                            if (listWord.size() < 4) { //Nếu số từ trong câu nhỏ hơn 4
                                setError(processEntity, childEntity, "Không đưa ra được số cược xiên. Vì tránh trường hợp tràn bộ nhớ nên Xiên ghép 4 chỉ chấp nhận từ 4 đến 15 cặp số. Các cặp số đó không được giống nhau");
                                isError = true;
                            } else {
            ...
            if (!isError) {
                if (listNum.size() < 4) {
                    ...
                    for (int i = 0; i < listNum.size() - 3; i++) {
                        for (int j = i + 1; j < listNum.size() - 2; j++) {
                            for (int k = j + 1; k < listNum.size() - 1; k++) {
                                for (int l = k + 1; l < listNum.size(); l++) {
                                    ...
                                    numberObject.setValue4(listNum.get(l));
                                    ...
                                    xienList.add(Integer.parseInt(listNum.get(l)));

...
public class ProcessStartWithXien3 {
    public static void processStartWithXien3(StringProcessEntity processEntity, StringProcessChildEntity childEntity, List<String> listWord, AccountObject customer, StringProcessChildEntity previousEntity,int position) {
        try {
                                ...
                                for (int i = 1; i < listWord.size() - 2; i++) { //Lấy các cặp số
                                    ...
                                    if (patternNumber.matcher(word).matches()) { //Nếu từ là số
                                        if (word.length() < 2) {
                                            ...
                                            if (word.length() == 2) {
                                                if (isDuplicateNum(listNum, word, 3)) {
                                                ...
                                                if (Pattern.compile("(\\d)\\d*\\1").matcher(word).matches()) { //Nếu là số đối xứng
                                                    if (isDuplicateNum(listNum, word.substring(0, 2), 3)) {
                                                        setError(processEntity, childEntity, "Các cặp số của xiên không được trùng nhau");
                                                        isError = true;
                                                        break;
                                                    } else {
                                                    ...
                                                    if (isDuplicateNum(listNum, word.substring(1), 3)) {
                                                        setError(processEntity, childEntity, "Các cặp số của xiên không được trùng nhau");
                                                        isError = true;
                                                        break;
                                                    } else {
                                                ...
                                                if (isDuplicateNum(listNum, word.substring(2), 3)) {
                                                    setError(processEntity, childEntity, "Các cặp số của xiên không được trùng nhau");
                                                    isError = true;
                                                    break;
                                                } else {
            ...
            if (!isError) {
                if (listNum.size() < 3 || listNum.size() % 3 != 0) {
                    setError(processEntity, childEntity, "Xiên 3 không đúng số cặp");
                } else {
                    ...
                    for (int i = 0; i < listNum.size(); i += 3) {
                        ...
                        xienList.add(Integer.parseInt(listNum.get(i + 2)));

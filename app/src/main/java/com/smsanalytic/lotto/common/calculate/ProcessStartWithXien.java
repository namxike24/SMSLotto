...
public class ProcessStartWithXien {
    public static void processStartWithXien(StringProcessEntity processEntity, StringProcessChildEntity childEntity, List<String> listWord, AccountObject customer, StringProcessChildEntity previousEntity,int position) {
        try {
                    ...
                    if (patternPrice.matcher(price).matches()) { //Nếu đúng định dạng giá trị tiền
                        ...
                        if (priceValue < 0) { //Nếu giá trị tiền = 0
                            ...
                            if (listWord.size() < 4) { //Nếu số từ trong câu nhỏ hơn 4
                                setError(processEntity, childEntity, "Xiên không đúng định dạng. Cần ghi rõ là xien2, xien3, xiên4");
                                isError = true;
                            } else {
            ...
            if (!isError) {
                if (listNum.size() < 2) {
                    ...
                    if (listNum.size() == 2) {
                        type = TypeEnum.TYPE_XIEN2;
                        childEntity.setType(type);
                        childEntity.setValue(TypeEnum.getStringByType(type) + childEntity.getValue().substring(childEntity.getValue().indexOf(" ")));

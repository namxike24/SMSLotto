...
import static com.smsanalytic.lotto.common.StringCalculateUtils.getListCangGiua;
import static com.smsanalytic.lotto.common.calculate.StringCalculate.*;

public class ProcessStartWithCangGiua {
    public static void processStartWithTypeCangGiua(StringProcessEntity processEntity, StringProcessChildEntity childEntity, List<String> listWord, StringProcessChildEntity previousEntity,int position) {
        try {
            ...
            int type = TypeEnum.TYPE_CANGGIUA;
                ...
                if (listWord.lastIndexOf("x") == listWord.size() - 1) { //Nếu x ở cuối cùng
                    ...
                    if (patternPrice.matcher(price).matches()) { //Nếu đúng định dạng giá trị tiền
                        ...
                        if (priceUnit == PriceUnitEnum.PRICE_D) {
                            setError(processEntity, childEntity, "Càng giữa phải kết thúc bằng n");
                            isError = true;
                                ...
                                if (listWord.size() < 4) { //Nếu số từ trong câu nhỏ hơn 4
                                    ...
                                    for (int i = 1; i < listWord.size() - 2; i++) { //Lấy các cặp số
                                        ...
                                        if (patternNumber.matcher(word).matches()) { //Nếu từ là số
                                            if (word.length() > 2) {
                                                setError(processEntity, childEntity, "Nhập nhằng 3 càng và càng nên sau càng chỉ chấp nhận 2 đơn vị, thay vì càng 345 x50n thì phải sửa lại thành cang 34 x 50n cang 5x50n");
                                                isError = true;
                                                break;
                                            } else {
                                                listNum.add(word);
...
//
            ...
            if (!isError) {
                for (String s : getListCangGiua(listNum)) {

...
public class ProcessStartWithXien4 {
    public static void processStartWithXien4(StringProcessEntity processEntity, StringProcessChildEntity childEntity, List<String> listWord, AccountObject customer, StringProcessChildEntity previousEntity,int position) {
        try {
                                ...
                                for (int i = 1; i < listWord.size() - 2; i++) { //Lấy các cặp số
                                    ...
                                    if (patternNumber.matcher(word).matches()) { //Nếu từ là số
                                        if (word.length() < 2) {
                                            ...
                                            if (word.length() == 2) {
                                                if (isDuplicateNum(listNum, word, 4)) {
                                                ...
                                                if (Pattern.compile("(\\d)\\d*\\1").matcher(word).matches()) { //Nếu là số đối xứng
                                                    if (isDuplicateNum(listNum, word.substring(0, 2), 4)) {
                                                        setError(processEntity, childEntity, "Các cặp số của xiên không được trùng nhau");
                                                        isError = true;
                                                        break;
                                                    } else {
                                                    ...
                                                    if (isDuplicateNum(listNum, word.substring(1), 4)) {
                                                        setError(processEntity, childEntity, "Các cặp số của xiên không được trùng nhau");
                                                        isError = true;
                                                        break;
                                                    } else {
                                                ...
                                                if (isDuplicateNum(listNum, word.substring(2), 4)) {
                                                    setError(processEntity, childEntity, "Các cặp số của xiên không được trùng nhau");
                                                    isError = true;
                                                    break;
                                                } else {
            ...
            if (!isError) {
                if (listNum.size() < 4 || listNum.size() % 4 != 0) {
                    ...
                    for (int i = 0; i < listNum.size(); i += 4) {
                        ...
                        xienList.add(Integer.parseInt(listNum.get(i + 3)));
